'''
sumalute every epoch need time
'''
import argparse
import logging
import os
...
from models import Mnist, LSTMCNN, MLP


def main():
    parser = argparse.ArgumentParser(description='PyTorch Training: Parkinson diagnose')
    parser.add_argument('--net-model', metavar='N', type=str, default='lstmcnn',
    ...
    parser.add_argument('-b', '--batch-size', metavar='N', type=int, default=64,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print('Training complete in {:.6f}s'.format(time_elapsed))
    print('mean_time every epoch cost: {:.6f}'.format(sum(epoch_time[2:-2])/len(epoch_time[2:-2])))
